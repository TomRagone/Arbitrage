/**
 * Phase 10C — committed search #2 on the same question as 10C-001.
 * Pre-registration record: docs/preregistration/10C-002-depth1-rsi-ema.md
 * (committed before this ran). Repairs 10C-001's data-availability setup
 * precondition (745-bar OHLC-endpoint ceiling) via the Phase 10C.1
 * trades-resampled series — not a re-roll of any multiplicity knob (§2/§5).
 *
 * Reads ONLY from the local sqlite store via the existing
 * @sol-edge/database read path, filtered to source = 'trades_resampled' —
 * deliberately does NOT call the live Kraken OHLC endpoint (that's exactly
 * what produced 10C-001's 745-bar ceiling).
 *
...
async function main() {
  ...
  console.log(`\nCommitted search space: ${candidates.length} candidates (pre-registered, docs/preregistration/10C-002-depth1-rsi-ema.md).\n`);
  ...
  if (significant) {
    ...
    console.log(
      `\nNo candidate cleared significance at the committed budget (264 trials, DSR >= 0.95, min 10 pooled OOS trades). This is a valid, complete null result — the holdout is NOT touched.`,
    );
  }

  writeSearchResultJson(
    "10C-002-depth1-rsi-ema",
    {
      runId: "10C-002",
      trials: top.trials,
      significant,
