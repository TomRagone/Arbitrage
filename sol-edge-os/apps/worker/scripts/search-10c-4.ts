...
async function main() {
  ...
  // Holding periods for the top candidate (whipsaw-churn diagnostic, kept
  // as structured data so the dashboard's verdict logic can detect it
  // without parsing prose).
  const topHoldingPeriods: number[] = [];
  plan.folds.forEach((fold) => {
    const fullSeries = [...fold.train, ...fold.test];
    const testFeatures = computeFeatures(fold.test, fullSeries, allFeatureKeys);
    const trades = runAstKernel(top.strategy, fold.test, testFeatures);
    for (const trade of trades) topHoldingPeriods.push(trade.exitTime - trade.entryTime);
  });

  writeSearchResultJson(
    "10C-004-breakout-momentum",
    {
      runId: "10C-004",
      trials: top.trials,
      significant,
